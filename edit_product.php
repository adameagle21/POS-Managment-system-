...
// Check permission - only users with edit permission or admin can access
if(!hasPermission('products_edit') && !hasRole('admin')) {
...
if(isset($_POST['submit'])) {
    ...
    if($price_last > 0 && $price_selling < $price_last) {
        ...
        if(empty($error)) {
            $query = "UPDATE products SET product_name='$name', sku='$sku', unit='$unit', category_id=$category_id, quantity='$quantity', alert_quantity='$alert_qty', image='$image', price_regular='$price_regular', price_selling='$price_selling', price_last='$price_last' WHERE id=$id";
...
<head>
    ...
    <title>Edit Product - Adam Car</title>
    ...
    <style>
        *{margin:0;padding:0;box-sizing:border-box;}
        ...
        .btn-update{background:linear-gradient(135deg,#f59e0b,#d97706);color:white;padding:12px 30px;border:none;border-radius:12px;font-weight:600;transition:0.3s;cursor:pointer;}
        .btn-update:hover{transform:translateY(-2px);box-shadow:0 5px 15px rgba(245,158,11,0.4);}
        ...
        .current-image{margin-top:5px;font-size:0.75rem;color:#64748b;}
...
<div class="main-content">
    ...
    <div class="form-container">
        ...
        <form method="POST" enctype="multipart/form-data">
            <div class="row">
                ...
                <div class="col-md-3">
                    <div class="form-group">
                        ...
                        <select name="unit" class="form-control">
                            ...
                            <option value="litter" <?= $product['unit']=='litter'?'selected':'' ?>>Litter</option>
                        </select>
                    </div>
                </div>
                <div class="col-md-3">
                ...
                <div class="col-md-4">
                    <div class="form-group">
                        ...
                        <input type="number" step="0.01" name="price_regular" class="form-control" id="price_regular" value="<?= $product['price_regular'] ?>" onkeyup="validatePrices()">
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="form-group">
                        <label>Selling Price ($) *</label>
                        <input type="number" step="0.01" name="price_selling" class="form-control" required id="price_selling" value="<?= $product['price_selling'] ?>" onkeyup="validatePrices()">
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="form-group">
                        <label>Last Price ($)</label>
                        <input type="number" step="0.01" name="price_last" class="form-control" id="price_last" value="<?= $product['price_last'] ?>" onkeyup="validatePrices()">
                    ...
                    <div class="form-group">
                        ...
                        <?php if($product['image']): ?>
                            <div class="current-image">
                                <i class="fas fa-image me-1"></i> Current image: <?= $product['image'] ?>
                                <br><img src="assets/uploads/<?= $product['image'] ?>" width="50" height="50" style="object-fit:cover; border-radius:8px; margin-top:5px;">
                            </div>
                        <?php endif; ?>
            ...
            <div class="mt-4">
                <button type="submit" name="submit" class="btn-update" id="submitBtn"><i class="fas fa-save me-2"></i>Update Product</button>
